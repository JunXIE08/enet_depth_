...
class Nyu40(data.Dataset):
	...
	def __init__(self, root_dir, mode='train', transform=None, label_transform = None, \
		...
		self.seg_classes = seg_classes #类别
		...
		if self.load_depth is True:
			self.loader = utils.nyu_loader_depth
		...
		if self.mode.lower() == 'train':
			...
			color_images, depth_images, labels = utils.get_filenames_nyu(self.root_dir+'/train/')
			...
			color_images, depth_images, labels = utils.get_filenames_nyu(self.root_dir+'/val/')
			...
			color_images, depth_images, labels = utils.get_filenames_nyu(self.root_dir+'/test/')
