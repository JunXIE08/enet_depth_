...
def get_filenames_nyu(base_dir):
    ...
    for path, _, files in os.walk(os.path.join(base_dir,'color')):
    #files = natsorted(files)
        for file in files:
            filename,_ = os.path.splitext(file)
            colorfile = os.path.join(base_dir,'color',filename+'.jpg')
            color_images.append(colorfile)
        
    for path, _, files in os.walk(os.path.join(base_dir,'label')):
    #files = natsorted(files)
        for file in files:
            filename,_ = os.path.splitext(file)
            labelfile = os.path.join(base_dir,'label',filename+'.png')
            labels.append(labelfile)
        
    for path, _, files in os.walk(os.path.join(base_dir,'depth')):
    #files = natsorted(files)
        for file in files:
            filename,_ = os.path.splitext(file)
            depthfile = os.path.join(base_dir,'depth',filename+'.png')
            depth_images.append(depthfile)
#储存图片（RGB图像为jpg格式，深度和label图像为png格式）
#确定文件的标签是否一一对应
